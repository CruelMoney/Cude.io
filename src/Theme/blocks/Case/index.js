	...
	handleMouseMove = (event, theRef) => {
		if (!event.setActive && !this.active) {
			if (!this.buttonHover) {
				...
				var x =
					(event.clientX - (rect.top + rect.width / 2)) / (rect.width / 2);
				var y =
	...
	applyImageTransformation = (img, translate, frontImg) => {
		if (frontImg) {
			...
			img.style.webkitTransform =
				img.style.webkitTransform + ` translateY(50px)`;
			setTimeout(() => {
				img.style.opacity = 1;
			}, 300);
		}
							...
							<div className={isApp ? "case-images-app" : "case-images"}>
								{this.props.case.images.map((img, idx) => {
									return (
										<LoadingImage
											key={
												"case-image-" + this.props.case._id + "-" + (idx + 1)
											}
